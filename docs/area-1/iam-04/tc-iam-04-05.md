# TC-IAM-04-05 - Login With Registered Email Containing Whitespace Boundaries

## Metadata

| Field | Value |
| :--- | :--- |
| Test Case ID | TC-IAM-04-05 |
...
Verify that submitting a valid registered email address with leading and/or trailing whitespace characters into the login form is sanitized and trimmed by the application, authenticates successfully without rejection, establishes an active session, displays the `"Logged in as <username>"` indicator, and deletes the account cleanly during teardown.

## Preconditions

1. The SUT is available.
2. The test starts with a fresh Playwright browser context and no authenticated session.
3. An active user account with a clean email address is pre-registered in the SUT (seeded via `POST /api/createAccount`).
...
Precondition account data (seeded via API with clean trimmed email):

- Name: `QA Trim User <timestamp>`
...
- Email: `  user_<yyyyMMdd_HHmmss_SSS>_<workerIndex>@qa.test  ` (seeded email flanked with leading and trailing spaces)
- Password: `Password@123` (exact match to seeded password)

Retain the clean generated email and password for API seeding, UI login, and cleanup. Never commit credentials to source control.

## Test Steps And Expected Results

1. Seed the user account via `POST /api/createAccount` with the clean email payload. Verify the response is HTTP `200` with JSON `responseCode` `201`.
...
4. Enter the whitespace-padded email (`  user_<...>@qa.test  `) into the login `Email Address` field. Verify the field accepts the input with space characters.
...
8. Verify the header navbar displays `Logged in as QA Trim User <timestamp>`.
...
The application trims leading and trailing whitespace from the email input, resolves the user identity accurately, logs the user in, renders `"Logged in as <username>"`, `Logout`, and `Delete Account` links, and the account is deleted cleanly during teardown.
...
3. If UI cleanup is unavailable or fails after account creation, call `DELETE /api/deleteAccount` with the clean seeded email and password in an `afterEach` hook.
...
- Use `loginPage.emailInput.fill('  ' + email + '  ')` to pass literal spaces into the DOM without framework auto-trimming.

...
| [**TC-IAM-02-03**](tc-iam-02-03.md) | Equivalence Partitioning | Duplicate email with uppercase or mixed-case local and domain parts (e.g., `EXISTING_USER@QA.TEST`) | Submission is blocked due to case-insensitive email collision handling, displaying the duplicate error banner | P2 | No |
| [**TC-IAM-02-04**](tc-iam-02-04.md) | Boundary Value Analysis | Duplicate email with leading and/or trailing whitespace characters (e.g., ` existing_user@qa.test `) | Input is either sanitized/trimmed and blocked as duplicate, or validated according to SUT input sanitization rules | P2 | No |
...
| [**TC-IAM-02-07**](tc-iam-02-07.md) | Error Guessing | Rapid repeated submission: Trigger duplicate signup button multiple times consecutively without reload | Form remains idempotent, retaining the error banner without duplicate network conflicts or unhandled UI exceptions | P2 | No |
